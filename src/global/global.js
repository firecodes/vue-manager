...
export  const global = {
        ...
        changeTheme:function(themeValue){
            ...
            //加载路径
            var itemPath = that.staticPath+'theme/'+themeValue+'/index.css';
            loadCss(itemPath);
            function loadCss(path){
                var link = document.createElement('link');
                    ...
                    link.rel = 'stylesheet';
                    link.type = 'text/css';
                    link.id = "theme-"+themeValue;
                    head.appendChild(link);
                link.addEventListener('load', function(){
                    //body 加id 
                    toggleClass(document.body, link.id);
                    //删除以前样式
                    var oldThemeValue = localStorage.getItem("themeValue");
                    if(oldThemeValue && oldThemeValue != themeValue){
                        delOldCss(oldThemeValue);
                    }
                    //设置新主题
                    localStorage.setItem("themeValue",themeValue);
                }, false);
            }
            function delOldCss(oldThemeValue){
                if(oldThemeValue){
                    var linkId = "theme-"+oldThemeValue;
                    var linkDom = document.getElementById(linkId);
                    head.removeChild(linkDom);
                    toggleClass(document.body, linkId);
                }
            }
        },
        /**
