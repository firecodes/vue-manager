    ...
    <div>
        ...
        <template v-if="routes">
            ...
            <el-submenu index="错误页面" v-if="routes['/errorpage/401'] || routes['/errorpage/404']">
                <template slot="title">
                    <i class="fa fa-bug" aria-hidden="true"></i> {{ $t('common.ereorPage404') }}
                </template>
                <router-link class="menu-indent" to="/errorpage/401" v-if="routes['/errorpage/401']">
                    <el-menu-item index="/errorpage/401">401</el-menu-item>
                </router-link>
                <router-link class="menu-indent" to="/errorpage/404" v-if="routes['/errorpage/404']">
                    <el-menu-item index="/errorpage/404">404</el-menu-item>
                </router-link>
            </el-submenu>
            ...
            <el-submenu index="综合实例" v-if="routes['/example/form'] || routes['/example/tinymce'] || routes['/example/mixin'] ">
                <template slot="title">
                    <i class="fa fa-envelope-open-o" aria-hidden="true"></i> {{ $t('common.synDemo') }}
                </template>
                <router-link class="menu-indent" to="/example/form" v-if="routes['/example/form'] ">
                    <el-menu-item index="/example/form">{{ $t('common.formEdit') }}</el-menu-item>
                </router-link>
                <router-link class="menu-indent" to="/example/tinymce" v-if="routes['/example/tinymce']">
                    <el-menu-item index="/example/tinymce">{{ $t('common.RichTxtEdit') }}</el-menu-item>
                </router-link>
                <router-link class="menu-indent" to="/example/mixin" v-if="routes['/example/mixin']">
                    <el-menu-item index="/example/mixin">{{ $t('common.subGroup') }}</el-menu-item>
                </router-link>

                <!-- 三级菜单 -->
                <el-submenu index="table内编辑">
                    <template slot="title">{{ $t('common.secondMenu') }}</template>
                    <router-link class="menu-indent" to="/example/31">
                        <el-menu-item index="/example/31">{{ $t('common.noRole401') }}</el-menu-item>
                    </router-link>
                    <router-link class="menu-indent" to="/example/table2">
                        <el-menu-item index="/example/table2">{{ $t('common.ereorPage404') }}</el-menu-item>
                    </router-link>
                </el-submenu>
            </el-submenu>
            ...
            <el-submenu index="系统设置" v-if="routes['/systemSet/permissionsManage'] || routes['/systemSet/loginLog']">
                <template slot="title">
                    <i class="fa fa-cogs fa-fw" aria-hidden="true"></i> {{ $t('common.systemSet') }}
                </template>
